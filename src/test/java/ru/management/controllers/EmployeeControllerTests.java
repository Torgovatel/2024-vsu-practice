...
class EmployeeControllerTest {
    ...
    void getById_ValidId_ReturnsEmployee() {
        testData.getValid().forEach(validEmployeeDTO -> {
            ...
            ResponseEntity<EmployeeDTO> response = employeeController.getEmployeeById(Long.toString(id));
            assertEquals(HttpStatus.OK, response.getStatusCode());
    ...
    void getById_InvalidId_ReturnsBadRequest() {
        ...
        ResponseEntity<EmployeeDTO> response = employeeController.getEmployeeById(invalidId);
        ...
        response = employeeController.getEmployeeById(invalidId);
        assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
    ...
    void getById_NonExistingId_ReturnsNotFound() {
        ...
        ResponseEntity<EmployeeDTO> response = employeeController.getEmployeeById(Long.toString(nonExistingId));
        assertEquals(HttpStatus.NOT_FOUND, response.getStatusCode());
        verify(employeeService).getEmployeeById(nonExistingId);
    }

    @Test
    void updateEmployeeById_ValidData_ReturnsNoContent() {
        testData.getValid().forEach(validEmployeeDTO -> {
            long id = validEmployeeDTO.getId();
            doNothing().when(employeeService).updateEmployeeById(eq(id), any(EmployeeDTO.class));
            ResponseEntity<Void> response = employeeController.updateEmployeeById(Long.toString(id), validEmployeeDTO);
    ...
    @Test
    void updateEmployeeById_InvalidData_ReturnsBadRequest() {
        testData.getInvalid().forEach(invalidEmployeeDTO -> {
            ...
            ResponseEntity<Void> response = employeeController.updateEmployeeById(Long.toString(id), invalidEmployeeDTO);
            assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
    ...
    @Test
    void updateEmployeeById_NonExistingId_ReturnsNotFound() {
        long nonExistingId = 999L;
        ...
        ResponseEntity<Void> response = employeeController.updateEmployeeById(Long.toString(nonExistingId), updatedEmployee);
        assertEquals(HttpStatus.NOT_FOUND, response.getStatusCode());
    ...
    void deleteById_ValidId_ReturnsNoContent() {
        testData.getValid().stream().map(EmployeeDTO::getId).forEach(id -> {
            ...
            ResponseEntity<Void> response = employeeController.deleteEmployeeById(Long.toString(id));
            assertEquals(HttpStatus.NO_CONTENT, response.getStatusCode());
    ...
    void deleteById_InvalidId_ReturnsBadRequest() {
        String invalidId = "abc";
        ResponseEntity<Void> response = employeeController.deleteEmployeeById(invalidId);
        assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
        ...
        response = employeeController.deleteEmployeeById(invalidId);
        assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
    ...
    void deleteById_NonExistingId_ReturnsNotFound() throws NotFoundException, DBAccessException {
        ...
        ResponseEntity<Void> response = employeeController.deleteEmployeeById(Long.toString(nonExistingId));
        assertEquals(HttpStatus.NOT_FOUND, response.getStatusCode());
